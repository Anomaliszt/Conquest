...
def create_agent_token(agent_id):
    """Creates a JWT token for the given agent ID with a 15-minute expiration."""
    now = time_now()
...
def create_agent_refresh_token_value():
    """Generates an opaque refresh token value (secure random bytes)."""
